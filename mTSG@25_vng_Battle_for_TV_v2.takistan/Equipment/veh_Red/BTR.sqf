_veh = _this select 0;

_veh addItemcargoglobal ["rhs_30Rnd_545x39_7N22_plum_AK", 30]; 
_veh addItemcargoglobal ["rhs_100Rnd_762x54mmR_7N13", 6];
_veh addItemcargoglobal ["rhs_VOG25", 20];
_veh addItemcargoglobal ["rhs_mag_rgd5", 10];
_veh addItemcargoglobal ["rhs_mag_rdg2_white", 15];
_veh addItemcargoglobal ["ACE_morphine", 7]; 
_veh addItemcargoglobal ["ACE_fieldDressing", 7]; 
_veh addItemCargoGlobal ["ACE_elasticBandage",7];
_veh addItemCargoGlobal ["ACE_packingBandage",7];
_veh addItemCargoGlobal ["ACE_quikclot",7];
_veh addItemCargoGlobal ["ACE_epinephrine",7];
_veh addBackpackCargoGlobal ["CUP_O_RUS_Patrol_bag_BeigeDigital", 2];
_veh addItemCargoGlobal ["rhs_weap_rpg26",2];																																																																																																																																																									
_veh addItemCargoGlobal ["rhs_rpg7_PG7VL_mag", 3]; 
_veh addItemCargoGlobal ["rhs_rpg7_OG7V_mag", 3]; 
_veh addItemCargoGlobal ["ToolKit", 1];
_veh addItemCargoGlobal ["rhs_weap_rshg2",1];